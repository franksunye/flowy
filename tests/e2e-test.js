...
class FlowyE2ETest {
  ...
  async testBasicFunctionality() {
    ...
    this.recordTest(
      'Create元素数量',
      createCount === TEST_CONFIG.expectations.createElementsCount,
      {
    ...
    const flowyAPI = await this.page.evaluate(
      () => typeof flowy !== 'undefined' && typeof flowy.output === 'function'
    );
  ...
  async testDragAndDropFunctionality() {
    ...
    for (const dragTest of TEST_CONFIG.dragTests) {
      ...
      try {
        ...
        this.recordTest(`拖拽${dragTest.name}`, false, {
          error: error.message,
        });
      }
    }
    ...
    console.log(
      `   📊 拖拽成功率: ${successfulDrags}/${TEST_CONFIG.dragTests.length}`
    );
  }
  ...
  async performAdvancedDrag(dragTest) {
    try {
      console.log(
        `     🎯 开始拖拽到吸附位置: (${dragTest.target.x}, ${dragTest.target.y})`
      );
  ...
  async verifySnapping(dragTest) {
    try {
      ...
      const blocks = await this.page.locator(
        TEST_CONFIG.snapping.snappedBlockSelector
      );
      ...
      // 验证块的标题
      const titleElement = latestBlock.locator(
        TEST_CONFIG.snapping.blockContentSelectors.title
      );
      ...
      if (titleCount > 0) {
        ...
        console.log(
          `     📝 块标题: "${titleText}" (期望包含: "${dragTest.expectedTitle}")`
        );
      ...
      const leftElement = latestBlock.locator(
        TEST_CONFIG.snapping.blockContentSelectors.left
      );
  ...
  async testWorkflowScenarios() {
    ...
    // 验证工作流状态
    const finalBlocksCount = await this.page
      .locator(TEST_CONFIG.snapping.snappedBlockSelector)
      .count();
    ...
    console.log(
      `     📊 工作流状态: ${finalBlocksCount}个块 (目标: ${targetBlocks}个)`
    );
    ...
    if (dataValid) {
      ...
      if (workflowData.data && workflowData.data.length > 0) {
        console.log(
          `     📋 第一个块数据:`,
          JSON.stringify(workflowData.data[0], null, 2)
        );
      }
    } else {
      console.log(
        `     ❌ 工作流数据无效: ${workflowData.error || '未知错误'}`
      );
    }
  ...
  async testTwoChildrenWorkflow() {
    ...
    // 验证工作流结构
    const structureValid = await this.verifyWorkflowStructure(
    ...
    // 验证并列节点重排
    const rearrangementValid = await this.verifyParallelNodeArrangement(
      scenario.expectedStructure.parallelChildren
    );
    ...
    const relationshipData =
      await this.verifyParentChildRelationships(scenario);
    this.recordTest(
      '一父二子关系数据',
      relationshipData.valid,
      relationshipData
    );

    console.log(
      `     📊 一父二子工作流: ${results.blocksCreated}个块创建，重排${rearrangementValid ? '成功' : '失败'}`
    );
  }
  ...
  async testThreeChildrenWorkflow() {
    ...
    console.log(
      `     📊 一父三子工作流: ${results.blocksCreated}个块创建，重排${rearrangementValid ? '成功' : '失败'}`
    );
  }
  ...
  async verifyWorkflowStructure(expectedStructure) {
    try {
      const actualBlocks = await this.page
        .locator(TEST_CONFIG.snapping.snappedBlockSelector)
        .count();
      const structureMatches = actualBlocks === expectedStructure.totalBlocks;

      console.log(
        `       📊 期望块数: ${expectedStructure.totalBlocks}, 实际块数: ${actualBlocks}`
      );
  ...
  async verifyParallelNodeArrangement(expectedParallelChildren) {
    try {
      ...
      // 找到最大的Y组（应该是并列子节点）
      const largestGroup = Object.values(yGroups).reduce(
        (max, group) => (group.length > max.length ? group : max),
        []
      );

      const actualParallelChildren = largestGroup.length;
      const arrangementValid =
        actualParallelChildren >= expectedParallelChildren;

      console.log(
        `       📊 期望并列节点: ${expectedParallelChildren}, 实际并列节点: ${actualParallelChildren}`
      );
      ...
      if (largestGroup.length > 1) {
        ...
        console.log(
          `       📊 并列节点水平排列: ${hasProperSpacing ? '正确' : '错误'}`
        );
  ...
  async verifyParentChildRelationships(scenario) {
    try {
      ...
      const maxParallelChildren = Math.max(
      ...
      const relationshipValid = {
        ...
        parallelChildrenMatch:
          maxParallelChildren >= scenario.expectedStructure.parallelChildren,
      };

      console.log(
        `       📊 父节点: ${parentBlocks.length}, 子节点: ${childBlocks.length}, 最大并列: ${maxParallelChildren}`
      );

      return {
        valid:
          relationshipValid.hasParent &&
          relationshipValid.hasChildren &&
          relationshipValid.parallelChildrenMatch,
