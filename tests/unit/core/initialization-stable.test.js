    ...
    describe('画布处理', () => {
        ...
        test('应该接受jQuery对象', async () => {
            await withIsolatedTest('jquery-object', async (testInstance) => {
                ...
                expect(() => {
                    testInstance.flowy($canvas, callbacks.grab, callbacks.release, callbacks.snapping);
                }).not.toThrow();
    ...
    describe('错误处理', () => {
        test('应该处理null画布参数', async () => {
            await withIsolatedTest('null-canvas', async (testInstance) => {
                ...
                // 调用本身不应该抛出错误
                expect(() => {
                    testInstance.flowy(null, callbacks.grab, callbacks.release, callbacks.snapping);
                }).not.toThrow();
