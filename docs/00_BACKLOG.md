...
- [ ] **1.2 移除内联块管理逻辑** (预计减少100行) 🎯 **最小粒度任务**
  - 具体位置: src/flowy.js 各种blocks数组直接操作代码 (不含重排)
...
- [ ] **2.2 替换直接jQuery调用** (预计减少代码复杂度)
  - 具体位置: src/flowy.js 107处 $() 直接调用
  - 操作: 将 $() 调用替换为 DomUtils.$()/相应方法
  - 验证: DOM操作功能保持一致，代码更规范
  - 预计工作量: 2-3小时
  - 风险: 低 (DomUtils已完整实现)

- [ ] **2.3 添加DomUtils模块加载** (预计10分钟)
  - 具体位置: src/flowy.js 模块加载部分
  - 操作: 添加getDomUtils()函数，类似其他模块
  - 验证: 模块正常加载，功能正常
  - 预计工作量: 10分钟
  - 风险: 极低
