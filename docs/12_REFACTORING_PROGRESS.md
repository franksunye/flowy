...
- ✅ 更新开发环境配置
  - 添加块管理模块导入
- ✅ 完成所有blocks数组操作替换
  - ✅ 替换 `blocks.length` 使用 → `getBlockCount()`
  - ✅ 替换 `Math.max` ID计算 → `getNextBlockId()`
  - ✅ 替换 `blocks = []` 清空 → `clearAllBlocks()`
  - ✅ 替换 `blocks.push()` 添加 → `addBlock()`
  - ✅ 替换 `$.merge()` 操作 → `mergeTempBlocks()`
  - ✅ 替换 `$.grep()` 过滤操作 → `removeBlockById()`
